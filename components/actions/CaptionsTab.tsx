...
import { Loader2, Sparkles, Type, Download, Save, Trash2, Globe } from "lucide-react";
...
export function CaptionsTab() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // Step 3: Client-side calibration
      // The WAV audio (resampled to 16kHz) has a different duration than the original video.
      // We must scale ALL timestamps by (videoDuration / totalWavDuration).
      // Using total WAV duration (not last word time) because there may be silence at the end.
      ...
      if (allCaptions.length > 0 && videoDuration > 0 && totalWavDuration > 0) {
        ...
        console.log(`[Captions] Calibration: WAV total=${totalWavDuration.toFixed(1)}s → Video=${videoDuration.toFixed(1)}s (scale=${scale.toFixed(4)}, drift=${((1-scale)*totalWavDuration).toFixed(1)}s)`);
        ...
        <div className="max-h-28 overflow-y-auto rounded-lg border border-border bg-surface-2">
          ...
          {filteredFonts.length === 0 && fontSearch && (
            <button
              ...
              className="w-full text-left px-3 py-1.5 text-xs text-accent hover:bg-surface-3"
            >
              Utiliser « {fontSearch} » (Google Font)
            </button>
          )}
        </div>
